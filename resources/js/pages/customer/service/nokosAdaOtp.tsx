...
export default function NokosAdaOtpOrder() {
    ...
    return (
        <div className="p-6 bg-black/30 space-y-6 backdrop-blur-sm rounded-lg">
            ...
            {!selectedParent && (
                <div>
                    <h2 className="text-lg font-semibold mb-4 text-gray-200">Pilih Platform (Ada OTP)</h2>
                                ...
                                >
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="w-12 h-12 bg-white rounded-lg p-2 flex items-center justify-center group-hover:scale-105 transition-transform">
                                            <img src={parent.icon} alt={parent.text} className="w-full h-full object-contain" />
                                        </div>
                                        <ChevronRight className="w-4 h-4 text-gray-500 group-hover:text-purple-400 transition-colors" />
                                    </div>
                                    <h3 className="text-sm font-semibold text-white mb-1">{parent.text}</h3>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-3 pb-4 border-b border-gray-600/30">
                        <div className="w-12 h-12 bg-white rounded-lg p-2 flex items-center justify-center">
                            <img src={selectedParent.icon} alt={selectedParent.text} className="w-full h-full object-contain" />
                        </div>
                        <div>
                            <h2 className="text-lg font-bold text-white">{selectedParent.text}</h2>
                    ...
                    <div className="space-y-3">
                        {filteredCountries.length > 0 ? (
                            filteredCountries.map((country: ServiceCountryAdaOtpProps) => (
                                <div
                                    key={country.id}
