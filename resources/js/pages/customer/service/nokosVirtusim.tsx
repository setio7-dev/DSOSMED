...
export default function NokosVirtusimOrder() {
    ...
    const filteredCountries = selectedChild
        ? selectedChild?.filter(c => Number(c.parent_id) === Number(selectedParent?.id))
        : [];
    ...
    return (
        <div className="p-6 bg-black/30 space-y-6 backdrop-blur-sm rounded-lg">
            ...
            {!selectedParent && (
                <div>
                    <h2 className="text-lg font-semibold mb-4 text-gray-200">Pilih Platform (Virtusim)</h2>
                                ...
                                >
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="w-12 h-12 bg-white rounded-lg p-2 flex items-center justify-center group-hover:scale-105 transition-transform">
                                            <img src={parent.img_link} alt={parent.text} className="w-full h-full object-contain" />
                                        </div>
                                        <ChevronRight className="w-4 h-4 text-gray-500 group-hover:text-purple-400 transition-colors" />
                                    </div>
                                    <h3 className="text-sm font-semibold text-white mb-1">{parent.country_name}</h3>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-3 pb-4 border-b border-gray-600/30">
                        <div className="w-12 h-12 bg-white rounded-lg p-2 flex items-center justify-center">
                            <img src={selectedParent.img_link} alt={selectedParent.country_name} className="w-full h-full object-contain" />
                        </div>
                        <div>
                            <h2 className="text-lg font-bold text-white">{selectedParent.country_name}</h2>
                    ...
                    <div className="space-y-3">
                        {filteredCountries.length > 0 ? (
                            filteredCountries.map((country: ServiceVirtusimListServiceProps) => (
                                <div
                                    key={country.id}
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                        <div className="flex items-center gap-3 flex-1">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <h3 className="text-sm font-semibold text-white mb-1">{country.name}</h3>
                                                <div className="flex items-center gap-3 text-xs">
                                                    <span className={`flex items-center gap-1 ${Number(country.tersedia) > 100 ? 'text-green-400' :
                                                        Number(country.tersedia) > 50 ? 'text-yellow-400' : 'text-red-400'
                                                        }`}>
                                        ...
                                        >
                                            ...
                                            {Number(country.tersedia) > 0 ? 'Order' : 'Habis'}
                                        </button>
                                    </div>
                                </div>
                            ))
