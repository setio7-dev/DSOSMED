...
export default function StatusSuntik() {
    const { transactionData, handleSuntikCheckStatus } = useTransactionHooks();
    ...
    const handleSelect = async (order: TransactionProps) => {
        ...
        setChecking(true);
        const result = await handleSuntikCheckStatus(order);
    ...
    const handleRecheck = async () => {
        ...
        setChecking(true);
        const result = await handleSuntikCheckStatus(selectedOrder);
        setStatusResult(result ?? null);
        setChecking(false);
    };
    ...
    return (
        <CustomerDashboard title="Status Suntik">
            <div className="space-y-6">
                <div>
                    <h1 className="text-xl font-bold text-white">Cek Status Suntik</h1>
                    <p className="mt-0.5 text-sm text-gray-400">Pilih pesanan untuk mengecek status terkini</p>
                </div>
                ...
                <div className="flex items-start gap-3 rounded-xl border border-blue-500/20 bg-blue-500/10 p-4">
                    ...
                    <div className="space-y-1">
                        <p className="text-sm font-semibold text-blue-300">Cara Cek Status</p>
                        <p className="text-xs leading-relaxed text-gray-400">
                            Pilih salah satu pesanan Suntik dari daftar di bawah untuk mengecek status terbarunya secara langsung dari provider.
                        </p>
                    </div>
                </div>
                ...
                {selectedOrder && (
                    <div className="rounded-xl border border-purple-500/20 bg-purple-500/10 p-4 space-y-4">
                        <div className="flex items-center justify-between">
                            <p className="text-sm font-semibold text-purple-300">Hasil Cek Status</p>
                            <button
                        ...
                        {checking ? (
                            <div className="flex items-center justify-center gap-2 rounded-lg border border-gray-600/30 bg-gray-800/50 py-4">
                                ...
                                <span className="text-sm text-gray-400">Mengecek status...</span>
                            </div>
                        ) : statusResult ? (
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <div className="rounded-lg border border-gray-600/30 bg-gray-800/50 px-3 py-2.5 space-y-1">
                                        <p className="text-xs text-gray-400">Nama Layanan</p>
                                        <p className="text-sm font-semibold text-white">{statusResult.nama}</p>
                                    </div>
                                    <div className="rounded-lg border border-gray-600/30 bg-gray-800/50 px-3 py-2.5 space-y-1">
                                    ...
                                    <div className="rounded-lg border border-gray-600/30 bg-gray-800/50 px-3 py-2.5 space-y-1">
                                        ...
                                        <p className="text-sm font-mono text-white truncate">{statusResult.target || '-'}</p>
                                    </div>
                                    <div className="rounded-lg border border-gray-600/30 bg-gray-800/50 px-3 py-2.5 space-y-1">
                                        <p className="text-xs text-gray-400">Jumlah</p>
                                        <p className="text-sm font-semibold text-white">{statusResult.jumlah}</p>
                                    </div>
                                    <div className="rounded-lg border border-gray-600/30 bg-gray-800/50 px-3 py-2.5 space-y-1 col-span-2">
                                        <p className="text-xs text-gray-400">Harga</p>
                                        <p className="text-sm font-semibold text-white">{FormatRupiah(statusResult.harga)}</p>
                                    </div>
                                </div>
                            </div>
                        ) : null}
                ...
                <div className="space-y-4 rounded-lg bg-black/30 p-6 backdrop-blur-sm">
                    <div className="flex items-center justify-between">
                        <h2 className="text-sm font-semibold text-gray-200 lg:text-lg">Pilih Pesanan</h2>
                        {suntikOrders.length > 0 && (
                            <span className="rounded-lg border border-purple-500/30 bg-purple-500/20 px-2.5 py-1 text-xs font-semibold text-purple-400">
                                {suntikOrders.length} order
                            </span>
                        )}
                    </div>
                    ...
                    {suntikOrders.length === 0 ? (
                        <div className="py-10 text-center">
                            ...
                            <p className="text-sm text-gray-400">Tidak ada pesanan Suntik ditemukan</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {suntikOrders.map((order: TransactionProps) => {
