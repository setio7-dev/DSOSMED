...
import { Pen, Trash, DollarSign, Package, Globe, Layers, Search, X } from 'lucide-react';
import React, { useEffect, useState } from 'react'

interface virtusimManagement {
    isOpen: boolean;
    onClose: () => void;
    service: ServiceVirtusimListServiceProps;
    country: ServiceVirtusimListCountryProps;
}
            ...
            <div className="bg-gray-900 border border-gray-700/50 rounded-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
                <div className="flex items-center justify-between p-6 border-b border-gray-700/50">
                    <h2 className="text-xl font-poppins-semibold text-white">
                        Ubah Layanan Ada OTP {country.country_name}
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                type="text"
                                name="name"
                                value={formPutVirtusim.name}
                                onChange={handleChangeVirtusimServiceUpdate}
                    ...
                    <div className="flex items-center justify-end gap-3 mt-6 pt-6 border-t border-gray-700/50">
                        ...
                        <button
                            type="submit"
                            onClick={() => handleUpdateVirtusimServiceCountry(Number(service.id))}
...
export default function VirtusimManagement() {
    ...
    const [filteredServices, setFilteredServices] = useState<ServiceVirtusimListCountryProps[]>([]);
    const [showAVirtusimModal, setShowAVirtusimModal] = useState(false);

    const [selectedServiceVirtusim, setSelectedServiceVirtusim] = useState<ServiceVirtusimListServiceProps | null>(null);
    const [selectedVirtusim, setSelectedVirtusim] = useState<ServiceVirtusimListCountryProps | null>(null);
    const { loading } = useAuth();

    useEffect(() => {
        const fetchFiltering = () => {
            const filter = customerserviceVirtusimData.filter((item) => {
                return item.country_name?.toLowerCase()?.includes(searchQuery.toLowerCase());
            })
        ...
        fetchFiltering();
    }, [searchQuery, customerserviceVirtusimData]);

    const handleVirtusimModal = (country: ServiceVirtusimListCountryProps, service: ServiceVirtusimListServiceProps) => {
        setShowAVirtusimModal(true);
        setSelectedServiceVirtusim(service)
        setSelectedVirtusim(country)
    }
    ...
    return (
        <div>
            <ServiceModal
                isOpen={showAVirtusimModal}
                onClose={() => setShowAVirtusimModal(false)}
                service={selectedServiceVirtusim as any}
                country={selectedVirtusim as any}
            />

            <div className="space-y-8">
                <div className="bg-gray-900/50 border border-gray-700/50 rounded-xl p-6">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                        <h2 className="text-xl font-semibold text-purple-300">Daftar Layanan Nokos Virtusim</h2>
                                        ...
                                        >
                                            <div className="flex items-center gap-3 flex-wrap">
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className="text-xs font-poppins-medium text-white">{FormatRupiah(Number(child.price))}</span>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className="text-xs font-poppins-regular text-gray-300">{child.tersedia}</span>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className="text-xs font-poppins-regular text-gray-300">{child.category}</span>
                                                </div>

                                                <div className="ml-auto flex items-center gap-2">
                                                    <button onClick={() => handleVirtusimModal(service, child)} className="p-1.5 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 rounded transition-all duration-200">
                                                        <Pen className="w-3.5 h-3.5" />
                                                    </button>
                                                    <button onClick={() => handleDeleteVirtusimServiceCountry(Number(child.id))} className="p-1.5 bg-red-600/20 hover:bg-red-600/30 text-red-400 rounded transition-all duration-200">
